...
```text
节点登录用户、端口和密码按节点保存，密码加密保存到 nodes.login_password_encrypted。
密码只用于首次测试和公钥分发，不出现在 API 响应、日志和任务快照中，API 仅返回 has_password 状态。
...
集群基础配置不再提供安装模式、操作系统和架构输入；系统固定执行离线安装。节点与登录配置页录入主机名、IPv4、IPv6、角色、SSH 用户、SSH 端口和登录密码，并提供“复制所选”和“测试全部节点”。测试全部节点会生成集群 SSH 密钥，按节点登录信息分发公钥、识别操作系统和架构，并保存最近一次测试结果。复制节点编辑保存后转为正式节点。存在草稿节点或测试结果失效时，预检查和安装入口必须禁用，后端也会拒绝请求。
