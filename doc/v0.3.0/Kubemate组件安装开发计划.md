...
- [x] 建立固定组目录，集中定义键、中文名称、包含组件、顺序和可用性。
- [x] 实现总开关和 `groups` 聚合响应。
- [x] 实现 `nfs`、`kubemate`、`traefik`、`storage_observability`、`prometheus`、`redis_sentinel` 六组。
- [x] Redis 哨兵标记为 unavailable，前后端绕过均不能启用。
- [x] 使用强类型 DTO 解析 NFS 配置，拒绝未知和非法字段。
- [x] 校验 NFS 地址、绝对路径、StorageClass 名称和管理模式。
- [x] 组件配置变化时递增配置版本并使组件预检查失效。
- [x] 活动任务期间拒绝组件配置写入。
- [x] 基础集群未安装时允许编辑所有未运行组。
- [x] 基础集群已安装时仅允许调整未安装或失败组；已安装和安装中组只读。
- [x] 返回稳定错误码，不向前端暴露数据库异常。
...
- [x] 用中文名称和组件清单替换英文键展示。
- [x] 增加“启用 Kubemate 组件安装”总开关。
- [x] 根据 `not_installed/installing/installed/failed` 展示明确状态。
- [x] 启用 NFS 后展示服务器、目录、StorageClass 和管理模式字段。
- [x] Redis 哨兵组禁用并显示“脚本待完善”。
- [x] 已安装组保持只读，关闭总开关时不能显示为已卸载。
- [x] 保存失败不进入下一阶段，后端字段错误定位到对应组。
- [x] 保持锁定集群中可查看组件事实状态。
- [x] 增加键盘操作、表单标签、错误提示和焦点可访问性测试。
...
- [x] 快照增加总开关、组件组、规范化配置、计划版本和介质校验和。
- [x] 验证快照 JSON 不包含凭据、Token 和 Secret 内容。
- [x] 新增 `job_type=component_install`。
- [x] 将组件补装纳入集群级活动任务互斥。
- [x] 在事务锁内完成配置版本检查、快照创建、状态更新和任务提交。
- [x] 任务接受时把计划内组设为 `installing`。
- [x] 每组成功后立即设为 `installed`，不等待后续组完成。
- [x] 当前组失败或应用中断时设为 `failed`，未开始组恢复 `not_installed`。
- [x] 保证重复回调不会让状态倒退或关联错误任务。
- [x] 组件任务不改变 Kubernetes 基础安装锁。
...
- [x] 先将现有 14 个 phase2 步骤无行为变化迁入基础计划工厂。
- [x] 建立固定组件组顺序和组内依赖定义。
- [x] 仅根据快照的有效启用组生成组件步骤。
- [x] 总开关关闭时组件计划为空，不分发 phase3 资源。
- [x] `storage_observability` 始终生成 OpenEBS、MinIO、Loki、Alloy 全部步骤。
- [x] 删除或禁用安装 API 的任意 `steps` 选择能力。
- [x] 将全局 `GET /api/install-plan` 调整为集群级权威计划预览。
- [x] 新集群安装拼接基础计划和组件计划。
- [x] 存量集群补装只生成未安装或失败组的组件计划。
- [x] 缺少目标节点、资源或前置条件时在任务创建前失败。
...
- [x] 统一 `tools/helm-amd` 与 `tools/helm-arm` 的架构映射。
- [x] 为 Helm 二进制建立 SHA-256 校验。
- [x] 根据主控制节点架构选择唯一资源。
- [x] 安装到 `/usr/local/bin/helm` 并记录 KubeFoundry 受管标记。
- [ ] 发现不兼容且非受管 Helm 时预检查失败，不静默覆盖。
- [x] 验证主控制节点的 `helm version` 和 `helm list -A`。
- [x] 组件资源分发到 `/tmp/kubefoundry/jobs/{jobId}/resources/{groupKey}`。
- [x] 只分发实际启用组资源，禁止依赖远端同路径的 `kube-media`。
- [x] 运行时统一设置 `KUBECONFIG=/etc/kubernetes/admin.conf`。
- [x] 清理任务临时资源时继续保留任务日志和校验和证据。
...
- [x] 安装概览区分 Kubernetes 基础状态和各组件组状态。
- [ ] 新集群安装确认展示基础步骤与已选组件组。
- [x] 存量集群提供组件预检查与“安装待安装组件”入口。
- [ ] 安装确认展示 Helm 目标节点、版本和非敏感组配置。
- [x] 任务执行页按基础、公共前置和组件组分段。
- [ ] 正确显示已成功组、失败组和未执行组。
- [ ] 失败后引导用户修正配置、重新预检查和重试。
- [x] 页面刷新和服务重启后恢复相同任务状态。
- [x] 活动任务期间所有冲突按钮和后端接口均拒绝重复提交。
...
- [x] 从最近安装快照和组件状态生成逆序清理计划。
- [x] 依赖逆序清理 Alloy、Loki、MinIO、OpenEBS。
- [x] 清理 Prometheus、Traefik、Kubemate 和 NFS 受管资源。
- [x] `/etc/fstab`、`/etc/exports` 只删除带受管标记的行。
- [x] external NFS 模式不触碰外部服务器。
- [x] 仅删除明确归属于任务快照的目录和 Kubernetes 资源。
- [x] Helm 只在受管标记和校验和匹配时删除。
- [x] 任一组件清理失败时保持集群锁，不进入成功解锁状态。
- [x] 全部重置成功后将组件状态恢复为 `not_installed`。
- [x] 对空路径、根目录、符号链接、越界和重复执行进行破坏性安全测试。
...
- [ ] 专项复核任务中断、重复重置和状态回写。
- [x] 先在临时目录和假 SSH 环境通过测试，再使用专用测试集群。
- [ ] 未完成专项复核不得合并或执行真实重置验收。

当前实现已通过离线伪命令和临时系统配置文件测试；远端删除、`kubeadm reset` 顺序和状态回写仍待 Sol 专项安全复核，禁止用于真实集群重置验收。
...
将版本统一升级为 0.3.0，确保发布包包含执行代码和双架构 Helm，完成自动化及 x86_64 真实环境验收。
...
- [x] 将后端、前端、健康接口、包名和部署文案统一升级为 0.3.0。
- [x] 发布包包含所有纳入计划的 phase3 脚本和验证脚本。
- [x] 发布包或部署目录能解析 amd64、arm64 Helm 介质及校验和。
- [x] 更新包内容、篡改检测、符号链接和缺失资源测试。
- [x] 扩展 Java Web 冒烟测试覆盖组件 API 与计划预览。
- [ ] 运行 Maven、Vitest、构建、Bash 测试、LF 和敏感信息检查。
- [ ] 按验收计划完成 x86_64 动态验收。
- [ ] 使用 5 台 x86_64 专用机器完成管理端、主控制节点和 Registry 同机拓扑验收。
- [ ] 使用指定 Worker 完成内部固定 NFS 验收，确认非受管配置和数据在安装及 reset 后保留。
- [x] ARM64 包结构、Helm 映射和架构隔离由自动化测试覆盖；真实环境验收登记为延期项。
- [x] 更新部署手册、使用手册、API 文档和最终验收清单。
- [x] 复核 Git 暂存范围、生成物和用户本地配置不进入提交。
...
| M5 安全与发布 | 任务 14 至任务 15 | 重置安全复核、双架构自动化校验与 x86_64 发布验收完成 |
...
- 每个组件组具有独立自动化测试和 x86_64 真实环境验收记录。
- 高风险重置和 NFS 系统配置通过专项安全复核。
- 新集群全量安装、v0.2.1 存量补装、失败重试和远程重置闭环通过。
- 管理端、主控制节点和 Registry 同机拓扑及内部固定 NFS 验收通过。
- ARM64 真实环境验收作为延期项记录，当前发布结论不得扩展到 ARM64 动态安装。
