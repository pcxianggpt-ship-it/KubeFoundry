...
- [x] 冻结续跑接口、准入错误码、步骤状态原因码和 SSE 事件字段。
...
- [x] 先覆盖验证退出码 `0/10/20/21/其他` 和前后验证组合测试。
- [x] 为步骤增加 `INSTALL/VALIDATION/MAINTENANCE` 类型及验证脚本路径。
- [x] 前置验证只上传 runtime、验证脚本和必要公共库，不解析大体积安装资源。
- [x] 前置验证为 `0` 时节点标记 `skipped/PREVERIFY_SATISFIED`。
- [x] 只有退出码 `10` 才解析、校验、上传资源并执行安装脚本。
- [x] 安装退出码为 0 后再次运行验证脚本；非 0 以 `POSTVERIFY_FAILED` 失败。
- [x] 验证异常和验证超时分别记录稳定原因，禁止进入安装。
- [x] 节点混合成功/验证跳过时正确汇总步骤状态。
- [x] 事件增加安全的 `verification_phase=before/after`，不新增不兼容终态。
- [x] 验证脚本、安装脚本、资源和输出证据分别保存 SHA-256。

任务 3 仅启用显式配置了验证脚本的严格双重验证；现有内联 `verifyCommand` 保持兼容执行，由任务 4 逐步迁移并开启全量严格验证。
...
- [x] 建立步骤与验证脚本一一对应的清单测试。
- [x] 移除验证脚本对 `PROJECT_ROOT`、`config.sh` 和嵌套 SSH 的依赖。
- [x] 所有验证只检查当前目标节点，并读取 `runtime.env` 白名单变量。
- [x] 统一 `0/10/20/21` 退出码和中文摘要。
- [x] 为 kubectl、helm、curl、systemctl、yum 和文件系统检查增加超时。
- [x] 将内联 `verifyCommand` 迁移到脚本，验证结果保持或增强。
- [x] 为 `18-init-k8s-cluster` 实现受控 Join 产物恢复，禁止输出 Token。
- [x] 检查全部验证脚本只读、幂等、无 CRLF、无交互命令。
- [x] 对同一模拟环境连续执行两次，验证结果一致。

任务 4 已于 2026-08-22 完成：当前最大安装计划中的 28 个 `INSTALL` 步骤均绑定同键验证脚本；最终集群健康检查保持 `VALIDATION` 类型。每个验证脚本直接包含当前步骤的只读检查逻辑，初始化步骤前置验证通过时会受控恢复本任务的 Join 产物。
...
- [x] 实现来源任务类型、终态、集群归属和快照完整性校验。
- [x] 拒绝成功任务、历史兼容任务、跨集群任务和活动任务续跑。
- [x] 校验当前节点身份、SSH 参数、架构和关键路径与来源快照一致。
- [x] 从来源快照重建同类型计划，不读取当前可变组件配置。
- [x] 创建新任务并保存 `source_job_id/run_mode=resume`。
- [x] 将来源介质校验信息复制到新任务快照；前置验证未满足后再校验实际资源。
- [x] 保证旧任务事件、日志和状态不被新任务回调修改。
- [x] 处理续跑任务再次失败后继续续跑的血缘链，禁止循环。
- [x] 返回稳定错误码和脱敏消息。

任务 5 已于 2026-08-23 完成：续跑在集群级事务锁内创建新任务，基础安装和组件安装均从来源 v0.3.2 快照重建完整计划。快照增加集群运行参数、逐节点安全运行路径、主机指纹和认证信息指纹；原始认证内容不进入快照。来源介质只绑定已保存 SHA-256，实际文件在步骤前置验证返回 `10` 后由执行器检查。
...
- [x] 增加续跑 API 客户端和契约测试。
- [x] 仅对允许状态和任务类型展示“续跑”按钮。
- [x] 点击后防止重复提交，接受后跳转新任务 ID。
- [x] 展示来源任务链接和运行模式，允许返回查看原失败任务。
- [x] 根据 `status_reason` 区分“已验证并跳过”和“因依赖跳过”。
- [x] 展示执行前验证、安装和执行后验证的安全阶段消息。
- [x] 续跑失败时保留当前页面和错误信息，不伪造新任务。
- [x] 覆盖刷新、任务切换、迟到 SSE 和终态重载。

完成记录（2026-08-23）：续跑入口、来源血缘、验证阶段状态和错误留页已经接入任务详情；API 客户端及安装流程测试覆盖重复提交、刷新切换、迟到 SSE 与终态展示。
