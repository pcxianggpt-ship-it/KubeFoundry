...
5. Kubemate 页面支持 MinIO PVC、CPU 和内存参数配置，且至少 4 个工作节点时才能启用 MinIO。
...
- [ ] 批准“续跑创建新任务，不原地恢复旧任务”。
- [ ] 批准验证退出码：`0=已满足`、`10=未满足`、`20=验证异常`、`21=验证超时`。
- [ ] 批准前置验证通过使用 `skipped + PREVERIFY_SATISFIED`，不新增数据库终态。
- [ ] 批准 v0.3.1 历史任务默认不可续跑，仅 v0.3.2 带完整快照和步骤键的任务可续跑。
- [ ] 批准初始化步骤通过独立产物恢复脚本重新生成短时 Join 产物。

门禁 A 未通过时，任务 2 至任务 6 不开始编码。
...
- [x] 冻结启用门禁：至少 4 个 `is_draft=false` 且包含 `worker` 角色的正式节点。
...
- [ ] 记录 Maven、Vitest、Bash、LF 和敏感信息检查的基线结果。
...
- [ ] Kubemate 页面加载正式 Worker 数量，少于 4 个时禁用包含 MinIO 的组件开关并显示当前数量。
- [ ] 后端组件保存接口重新统计正式 Worker，少于 4 个时返回 `MINIO_WORKER_COUNT_INSUFFICIENT`。
- [ ] 错误详情只返回 `required_workers=4` 和 `actual_workers`，不返回节点凭据或内部异常。
- [ ] 已启用后 Worker 数量减少时保留配置，但组件预检和安装准入失败。
...
- 0～3 个正式 Worker 时前端不能启用 MinIO，绕过前端调用后端也不能保存。
- 恰好 4 个正式 Worker 时，在其他配置合法的前提下可启用和保存。
...
- [ ] 安装确认页所有节点显示 IP。
- [ ] MinIO 实际资源与配置一致。
- [ ] MinIO 在 0～3 个 Worker 时无法启用，4 个 Worker 时可以启用。
- [ ] MinIO 启用后 Worker 数量降低时，预检和安装准入拒绝继续。
...
- MinIO 至少 4 个正式 Worker 的启用和保存门禁不可绕过，且不得误加 Ready Worker 数量门禁。
