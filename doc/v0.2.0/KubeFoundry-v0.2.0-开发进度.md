...
- 权限与安全：主密钥仅支持同时提供 POSIX 权限和 `SecureDirectoryStream` 的文件系统；新建文件在写入前复核并修正为 `rw-------`，无法验证时失败关闭并删除空文件。已有文件带组或其他用户权限、符号链接、身份变化或无法提供 `fileKey` 时均拒绝加载；解密失败不暴露敏感值，临时字节数组在使用后覆盖。
