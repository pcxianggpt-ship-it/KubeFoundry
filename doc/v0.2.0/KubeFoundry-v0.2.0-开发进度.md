...
- 并发一致性：同一集群的测试启动使用按集群粒度的原子准入，重复请求返回现有任务 ID；节点状态使用数据库条件更新和原子聚合，配置版本变化后丢弃旧连接产生的指纹、探测结果和成功状态。
- 验证：覆盖阶段顺序、系统信息、部分失败、失败节点重试、密码/API/SSE 脱敏、并发启动、配置变化、集群聚合、集群/单节点 API、幂等公钥命令和架构规范化；Java 后端全量 66 个测试通过，其中 14 个 POSIX 权限测试按 Windows 环境预期跳过。
