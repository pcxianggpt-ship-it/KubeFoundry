...
未通过门禁 A 时，任务 8 不进入实现；其他任务可继续开发，MinIO 自动化不得被标记为完成。
...
任务 2、任务 4、任务 5 和任务 6 在文件边界不冲突时可独立推进。任务 8 必须等待 MinIO 门禁 A；任务 9 必须等待状态契约门禁 B。
...
| 里程碑 | 任务 | 参考工作量 | 完成标志 |
| --- | --- | --- | --- |
| M0 决策与基线 | 任务 1、门禁 A/B/C | 1～2 人日 | 契约、迁移和风险结论可执行 |
| M1 配置与节点 | 任务 2～4 | 4～6 人日 | 节点唯一、批量测试、独立组开关可用 |
| M2 脚本可靠性 | 任务 5～8 | 7～11 人日 | CoreDNS、NFS、MinIO 自动化测试通过 |
| M3 组件隔离 | 任务 9～10 | 5～8 人日 | 组失败隔离、部分成功和前端展示闭环 |
| M4 任务历史 | 任务 11 | 3～5 人日 | 多任务 ID 可切换、刷新、实时更新 |
| M5 发布验收 | 任务 12 | 3～5 人日 | 全量测试、真实环境验收和文档完成 |
...
## 13. 任务 7：MinIO 版本和介质预检查

### 目标

在实施自动化部署前，建立固定版本、离线资源、配置和安全检查，避免脚本运行后才发现 Operator 不兼容。
...
- [ ] 预检查目标 Kubernetes 版本与固定 Operator 版本的兼容范围。
- [ ] 预检查 StorageClass、容量格式、Worker 数量和所需 PVC 数量。
- [ ] 仅将非敏感 MinIO 配置写入安装快照。
- [ ] 验证发布包不依赖在线 Chart、Kustomize URL 或镜像标签漂移。
...
- 离线资源版本、来源和校验和可审计。
- 配置不足或不兼容不会创建安装任务。
- 快照、事件和日志不包含 MinIO 凭据。

### 建议提交

```text
组件：增加MinIO离线介质与配置预检查
```

## 14. 任务 8：MinIO Operator 与 Tenant 自动化部署

### 目标

通过固定离线清单自动安装 Operator、创建凭据 Secret、应用 Tenant CR 并完成工作负载验证。
...
- 修改：`ComponentPlanFactory.java` 和相关测试。
- 修改：存储与日志 verify 脚本。

### 实施步骤

- [ ] 先覆盖 Operator 已存在/不存在、Secret 已存在/不存在、Tenant 重复应用测试。
- [ ] 应用固定 Operator、CRD 和 RBAC 清单并等待 CRD/Deployment 就绪。
- [ ] 首次安装从安全随机源生成凭据，通过标准输入创建 Secret。
- [ ] 已有 Secret 时复用，不重新生成或回显。
- [ ] 渲染任务目录中的 Tenant CR，不修改发布介质原文件。
- [ ] 应用 Tenant CR 并等待 Tenant 成功条件。
- [ ] 验证 Pod Ready、PVC Bound、Service 和 Loki 所需最小对象存储链路。
- [ ] 所有等待设置超时并输出脱敏诊断。
- [ ] MinIO 失败时返回非零，阻止本组 Loki/Alloy，但不影响其他组件组。
...
- 重复执行复用 Secret 和 Tenant，不产生冲突资源。
- PVC、Pod、Service 和对象存储链路验证全部通过。
- 故障注入时敏感信息不进入任何日志或任务数据。
...
## 18. 任务 12：集成、文档、打包与发布验收

### 目标

完成七项优化的联合回归、真实环境验收、发布包验证及中文文档同步。

### 主要文件

- 修改：`doc/v0.3.1/` 下设计、开发计划、接口说明、使用手册和验收清单。
...
- [ ] 验证 MinIO 离线 Operator/Tenant、PVC 和 Loki 最小对象存储链路。
- [ ] 生成至少三个安装任务并验证任务历史切换与刷新。
- [ ] 更新部署、使用、接口、验收和已知限制文档。
...
- [ ] 高风险 NFS、MinIO、安装状态机和迁移通过专项复核。
- [ ] 自动化测试全部通过，无未解释的基线回归。
- [ ] x86_64 真实集群验收通过。
- [ ] ARM64 至少完成介质、计划和脚本静态验证；无真实环境时明确记录延期。
- [ ] MinIO 上游归档风险、固定版本和支持范围写入发布说明。
...
6. `脚本：增强NFS挂载幂等与超时保护`
7. `组件：增加MinIO离线介质与配置预检查`
8. `组件：实现MinIO租户非交互部署`
9. `任务：隔离Kubemate组件组安装失败`
10. `前端：展示组件安装部分成功与跳过状态`
11. `任务：按任务ID展示集群安装历史`
12. `发布：完成v0.3.1优化功能验收`
