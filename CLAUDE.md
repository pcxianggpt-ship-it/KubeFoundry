...
2. **修改 Pod 清单**：添加'- --cluster-signing-duration=867240h0m0s'参数到 `spec.containers[0].command` 最后一行
3. **重启 Pod**：kubelet 会自动重启以应用新配置
4. **验证生效**：检查 Pod 状态和参数加载情况


#### 常用参数
- `--feature-gates`: 启用/禁用功能特性
- `--horizontal-pod-autoscaler-use-rest-clients`: HPA 客户端配置
- `--bind-address`: 监听地址配置
- `--node-cidr-mask-size`: 节点 CIDR 掩码大小
- `--pod-eviction-timeout`: Pod 驱逐超时时间
