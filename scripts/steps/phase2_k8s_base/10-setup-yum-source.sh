...
if [ ! -f "$repo_source_name" ]; then
    log_error "YUM源文件不存在: $repo_source_name"
    exit 1
fi
log_info "找到YUM源文件: $repo_source_name"
...
if [ $(yum -q search kubelet | wc -l) -gt "0" ]; then
    log_success "本地yum源已经安装"
    log_info "kubelet包搜索结果:"
