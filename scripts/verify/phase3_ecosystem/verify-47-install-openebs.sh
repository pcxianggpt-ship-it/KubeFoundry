...
while [ $wait_count -lt 12 ]; do
    running=$(kubectl get pods -n kubemate-system --no-headers 2>/dev/null | grep 'openebs' | grep -c 'Running' || true)
...
# 1. openebs 相关 Pod 存在
result=$(kubectl get pods -n kubemate-system --no-headers 2>/dev/null | grep 'openebs' | wc -l)
if [ "$result" -ge 1 ]; then
...
# 2. openebs Pod 运行状态
result=$(kubectl get pods -n kubemate-system --no-headers 2>/dev/null | grep 'openebs' | grep -c 'Running' || true)
if [ "$result" -ge 2 ]; then
...
result=$(kubectl get sc --no-headers 2>/dev/null | grep -c 'openebs' || true)
if [ "$result" -ge 1 ]; then
...
result=$(helm list -n kubemate-system --no-headers 2>/dev/null | grep -c 'openebs' || true)
if [ "$result" -ge 1 ]; then
