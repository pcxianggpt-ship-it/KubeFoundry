...
while [ $wait_count -lt 12 ]; do
    running=$(kubectl get pods -n kubemate-system --no-headers 2>/dev/null | grep 'traefik' | grep -v 'mesh' | grep -c 'Running' || true)
...
# 1. traefik 相关 Pod 存在
result=$(kubectl get pods -n kubemate-system --no-headers 2>/dev/null | grep 'traefik' | wc -l)
if [ "$result" -ge 1 ]; then
...
result=$(kubectl get pods -n kubemate-system --no-headers 2>/dev/null | grep 'traefik' | grep -v 'mesh' | grep -c 'Running' || true)
if [ "$result" -ge 1 ]; then
...
result=$(kubectl get svc -n kubemate-system --no-headers 2>/dev/null | grep -c 'traefik' || true)
if [ "$result" -ge 1 ]; then
