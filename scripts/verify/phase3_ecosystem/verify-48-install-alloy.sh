...
run env KUBECONFIG="${KF_KUBECONFIG}" helm status alloy --namespace kubemate-system >/dev/null 2>&1 || missing "alloy Helm Release 不存在"
...
[ "${status}" -eq 0 ] || error "alloy Pod 查询失败"
printf '%s\n' "${pods}" | awk '$1 ~ /alloy/ && ($3 == "Running" || $3 == "Completed") { found=1 } END { exit !found }' || missing "alloy 工作负载未就绪"
printf '[SUCCESS] alloy 已就绪\n'
